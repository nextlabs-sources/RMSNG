package com.nextlabs.router.security;
...
import com.nextlabs.router.hibernate.model.KeyStoreEntry;
import com.nextlabs.router.hibernate.model.KeyStoreEntry.KeyStoreType;
...
public class KeyStoreManagerImpl implements IKeyStoreManager {
    ...
    @Override
    public boolean containsAlias(String tenantName, String alias) throws GeneralSecurityException, IOException {
        KeyStoreEntry entry = getKeyStore(tenantName);
        KeyStore keyStore = toKeyStore(entry);
        return keyStore != null && keyStore.containsAlias(alias);
    }

    @Override
    public void createKeyStore(String tenantName, List<com.nextlabs.common.security.Entry> entries)
        ...
        for (com.nextlabs.common.security.Entry entry : entries) {
            ...
            if (!StringUtils.hasText(tenant)) {
                tenant = ROOT;
            } else if (tenant != null && !StringUtils.equals(tenantName, tenant)) {
    ...
    private KeyStoreEntry getKeyStoreEntry(DbSession session, String tenantName,
        List<com.nextlabs.common.security.Entry> entries)
            throws IOException, GeneralSecurityException {
        KeyStoreEntry keyStoreEntry = getKeyStore(session, tenantName);
        ...
        if (keyStoreEntry != null) {
            ...
            keyStoreEntry.setTenantName(tenantName);
        }
        for (com.nextlabs.common.security.Entry entry : entries) {
    ...
    private KeyStoreEntry getKeyStore(DbSession session, String tenantName) {
        Criteria criteria = session.createCriteria(KeyStoreEntry.class);
        if (StringUtils.hasText(tenantName)) {
            criteria.add(Restrictions.eq("tenantName", tenantName));
        } else {
            criteria.add(Restrictions.isNull("tenantName"));
        }
        return (KeyStoreEntry)criteria.uniqueResult();
    }

    @Override
    public Certificate getCertificate(String tenantName, String alias) throws GeneralSecurityException, IOException {
        KeyStoreEntry entry = getKeyStore(tenantName);
        KeyStore keyStore = toKeyStore(entry);
    ...
    @Override
    public Certificate[] getCertificateChain(String tenantName, String alias)
            throws GeneralSecurityException, IOException {
        KeyStoreEntry entry = getKeyStore(tenantName);
        KeyStore keyStore = toKeyStore(entry);
    ...
    @Override
    public Key getKey(String tenantName, String alias) throws GeneralSecurityException, IOException {
        KeyStoreEntry entry = getKeyStore(tenantName);
        KeyStore keyStore = toKeyStore(entry);
        if (keyStore != null) {
    ...
    private KeyStoreEntry getKeyStore(String tenantName) {
        try (DbSession session = DbSession.newSession()) {
            return getKeyStore(session, tenantName);
        }
    }

    @Override
    public SecretKey getSecretKey(String tenantName, String alias) throws GeneralSecurityException, IOException {
        KeyStoreEntry entry = getKeyStore(tenantName);
        KeyStore keyStore = toKeyStore(entry);
        if (keyStore == null) {
            return null;
    ...
    @Override
    public void deleteKeyStore(String tenantName) {
    }
