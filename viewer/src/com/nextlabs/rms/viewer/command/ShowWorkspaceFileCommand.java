...
public class ShowWorkspaceFileCommand extends AbstractCommand {
    ...
    public void doAction(HttpServletRequest request,
        ...
        Audit.audit(request, "Command", "ShowWorkspaceFile", "ShowWorkspaceFile", 0, uid, user.getTenantName(), filePath, filePathDisplay);
        try {
            ShowFileResult showFileResult = ViewFileManager.getInstance().showWorkspaceFile(filePath, filePathDisplay, user, offset, lastModifiedDateStr, viewingSessionId, request, response);
            ...
            ViewerUtil.setCookie("promptEWSDownload", promptDownload, request, response);
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("Error occurred when user opened file (user ID: {}, Workspace Tenant Id: {}, file path: {}): {}", uid, user.getTenantId(), filePath, e.getMessage(), e);
            }
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("IO Error occurred when user opened file (user ID: {}, Workspace Tenant Id: {}, file path: {}): {}", uid, user.getTenantId(), filePath, e.getMessage(), e);
            }
            ViewerUtil.sendErrorResponseWithStatusCode(request, response, viewingSessionId, "err.generic", 500);
        } catch (Exception e) {
            LOGGER.error("Error occurred when user opened file (user ID: {}, Workspace Tenant Id: {}, file path: {}): {}", uid, user.getTenantId(), filePath, e.getMessage(), e);
            ViewerUtil.sendErrorResponseWithStatusCode(request, response, viewingSessionId, "err.generic", 500);
        }
    }
}
