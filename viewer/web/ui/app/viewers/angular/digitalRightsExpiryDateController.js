mainApp.controller('digitalRightsExpiryDateController',[ '$scope', '$filter', '$animate', '$timeout', 'userPreferenceService', 'digitalRightsExpiryService',
    function($scope, $filter, $animate, $timeout, userPreferenceService, digitalRightsExpiryService) {
                    ...
                    if(data.statusCode == 200) {
                        if(data.results != undefined){
                            if(data.results.expiry){
                                var obj = JSON.parse(data.results.expiry);
                                parseExpiry(obj);
                            }
                        }
                    } else {
        ...
        $scope.$watch('expiry.startDatePicker',function(){
            ...
            $scope.minEndDate = new Date(new Date($scope.expiry.startDatePicker).setHours(0,0,0,0));
