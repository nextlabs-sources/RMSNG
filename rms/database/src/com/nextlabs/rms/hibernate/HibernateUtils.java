...
public final class HibernateUtils {
    ...
    public static synchronized void bootstrap(Properties prop, final boolean migrate) {
                ...
                public void execute(Connection connection) {
                    try {
                        if (migrate) {
                            ...
                            flyway.setLocations("db/migration", "db/" + databaseName, "db/java/");
                            flyway.migrate();
    ...
    public static long nextSequence(DbSession session, String seqName) {
        ...
        if (sf instanceof SessionFactoryImplementor) {
            SessionFactoryImplementor implementor = SessionFactoryImplementor.class.cast(sf);
            Dialect dialect = implementor.getDialect();
            if (dialect.supportsSequences()) {
                String queryStr = dialect.getSequenceNextValString(seqName);
                SQLQuery query = session.createSQLQuery(queryStr);
                Number result = (Number)query.uniqueResult();
                return result.longValue();
            } else {
                throw new UnsupportedOperationException("Database dialect doesn't support sequences.");
            }
        } else {
            throw new UnsupportedOperationException("Unsupported SessionFactory implementor.");
        }
    }
