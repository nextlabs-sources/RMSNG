...
public class ProtectAndUploadInPlaceCommand extends AbstractCommand {
    ...
    public void doAction(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (!StringUtils.hasText(repoId) || !StringUtils.hasText(filePath) || !StringUtils.hasText(filePathDisplay) || (!StringUtils.hasText(rightsGranted) && !StringUtils.hasText(tagsSelected))) {
        ...
        if (tagsSelected == null && (rightsGranted == null)) {
            jsonResponse.setError(RMSMessageHandler.getClientString("fileRightsErr"));
            JsonUtil.writeJsonToResponse(jsonResponse, response);
            return;
        }
        try {
        ...
        String[] rightsGrantedList = null;
        if (StringUtils.hasText(rightsGranted)) {
            rightsGrantedList = rightsGranted.split(",");
        }
        ...
        try {
            String membershipId = RepositoryFileUtil.getDynamicMembershipName(userPrincipal.getUserId(), userPrincipal.getTenantName());
            ...
            if (rights.length == 0 && !StringUtils.hasText(tagsSelected)) {
                jsonResponse.setError(RMSMessageHandler.getClientString("error.invalid.rights"));
                JsonUtil.writeJsonToResponse(jsonResponse, response);
                return;
            }
            fileMeta = RepositoryFileUtil.protectAndUploadInPlace(userPrincipal, repository, path, filePath, filePathDisplay, outputPath, membershipId, rights, watermark, expiry, userConfirmedFileOverwrite, request);
            fileFromRepo = fileMeta.getFile();
            jsonResponse.setName(fileMeta.getFileNameWithTimeStamp());
            jsonResponse.setFilePath(fileMeta.getPathId());
            jsonResponse.setFilePathDisplay(fileMeta.getPathDisplay());
            jsonResponse.setRepoId(fileMeta.getRepoId());
